...
<head>
    ...
    <title>{{ config('app.name') }} - {{ __('Admin Quên Mật Khẩu') }}</title>
    ...
    <div class="page page-center">
        <div class="container container-tight py-4">
            <div class="text-center mb-4">
                <a href="{{ auth()->check() ? route('admin.dashboard') : route('admin.login') }}" aria-label="Tabler"
            ...
            <div class="card card-md">
                <div class="card-body">
                    <p>
                        {{ __('Quên mật khẩu? Không sao cả. Chỉ cần cho chúng tôi biết địa chỉ email của bạn và chúng tôi sẽ gửi cho bạn một liên kết đặt lại mật khẩu qua email, cho phép bạn chọn một mật khẩu mới..') }}
                    </p>
                    ...
                    <form action="{{ route('admin.password.email') }}" method="POST" autocomplete="off" novalidate>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="email" name="email" :value="old('email')" class="form-control" placeholder="your@email.com" autocomplete="off" required autofocus />
                            <x-input-error :messages="$errors->get('email')" class="mt-2" />
                        </div>

                        <div class="form-footer">
                            <button type="submit" class="btn btn-primary w-100">{{ __('Liên kết đặt lại mật khẩu email') }}</button>
                        </div>
                    </form>
                </div>
            </div>
